...
- `/catalogo` — módulo unificado con tabs (`CatalogoTabs`):
  ...
  - **Características de artículo** — pares tipo → valor por artículo, en la ficha de `/catalogo/articulo/[id]`. Los tipos (`TipoCaracteristica`: nombre, `nombreClave` normalizado y único, unidad opcional) los crea el usuario y son transversales a todo el catálogo; el valor (`CaracteristicaProducto`, único por `(productoId, tipoId)`) es texto libre. UI en `CaracteristicasArticulo` (filas con valor editable inline) + `TipoCaracteristicaPicker`, que además es donde se crean, renombran y eliminan los tipos (no hay pantalla aparte). Acciones `crearTipoCaracteristica` / `actualizarTipoCaracteristica` / `eliminarTipoCaracteristica` / `listarTiposCaracteristica` / `guardarCaracteristica` (upsert) / `eliminarCaracteristica` en `catalogo/actions.ts`; lecturas `getCaracteristicasProducto` / `getTiposCaracteristica` en `src/lib/catalogo.ts`; tipos cliente-safe en `src/lib/caracteristicas-tipos.ts`.
